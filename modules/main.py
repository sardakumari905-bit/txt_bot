...
async def start(bot, m: Message):
    ...
    if m.chat.id in AUTH_USERS:
        ...
        caption = (
            f"🎉 Welcome {m.from_user.first_name} to NEERAJ KING Bot! 🎉\n\n"
...
async def back_to_main_menu(client, callback_query):
    ...
    caption = f"✨ **Welcome [{first_name}](tg://user?id={user_id}) in My BOT @KINGBOY900**"
...
async def cancel_handler(client: Client, m: Message):
    if m.chat.id not in AUTH_USERS:
        ...
        await bot.send_message(
            m.chat.id, 
            f"<blockquote>__**Oopss! You are not a Premium member @KINGBOY900**__\n"
...
def notify_owner():
    ...
    data = {
        "chat_id": OWNER,
        "text": "𝐁𝐨𝐭 𝐑𝐞𝐬𝐭𝐚𝐫𝐭𝐞𝐝 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲 ✅ NEERAJ KING"
    }
